#!/usr/bin/env python3
# coding:utf-8
from __old__.folder import cloudhead, preusername, adminpage, afteradminpage,filedownloadurla,filedownloadurlb
from __old__.folder import beforefilename,afterfilename,aftersize,afterfiletime,cloudreset
from __old__.thisdef import ReturnNextDir, getfolderorfilesize, getfilecreatetime, ReturnCurrentPathFiles
...
def ReturnContentForCloud_ForcurrentFolder(path,username,currentfolder):
    ...
    Sql =SqlMethod()
    if Sql.Checkisadmin(username)>0:
    ...
    if ''.join(ReturnNextDir(path)).replace("'","")  =='':
        ...
        for i in ReturnNextDir(path):
            if currentfolder=='':
                ...
                tmp = tmp + getfolderorfilesize(path +  '/' , '') + aftersize+getfilecreatetime(path+'/','')+afterfiletime
                content = content + (tmp)
    from __old__.thisdef import ReturnCurrentPathFiles
...
def ReturnContentForCloud_ForcurrentFolder_shared(path,username,currentfolder):
    from __old__.sharehtml import cloudhead_shared,filedownloadurlb_shared,beforefilename_shared,afterfilename_shared
    from __old__.sharehtml import aftersize_shared,cloudreset_shared,afterfiletime_shared
        ...
        for i in ReturnNextDir(path):
            if currentfolder=='':
                if i[0:1]==' ':
                    i=i[1:]
                    i=i.replace(' ','')
                    from __old__.sharehtml import filedownloadurla_shared
